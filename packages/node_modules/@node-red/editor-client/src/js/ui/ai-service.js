...
- Preserve IDs for updates`
    ...
    function prepareMessages(userMessage, conversationHistory, availableNodeTypes, nodesInWorkspace, maxContextTokens) {
        ...
        if (RED.aiRAGService && RED.aiRAGService.isInitialized()) {
            ...
            // For questions, use only RAG results (minimal context)
            if (operationType === 'question') {
                if (Object.keys(relevantNodeTypes).length > 0) {
                    contextNodeTypes = relevantNodeTypes;
                }
            } else {
                // For flow creation and updates, merge RAG results with all node types
                // This ensures all node types are available, but RAG-relevant ones are included
                // Start with all node types
                var mergedNodeTypes = {};
                
                // First, add all available node types
                for (var nodeType in availableNodeTypes) {
                    if (availableNodeTypes.hasOwnProperty(nodeType)) {
                        mergedNodeTypes[nodeType] = availableNodeTypes[nodeType];
                    }
                }
                
                // Then, ensure RAG-relevant types are included (they should already be, but this is a safety check)
                for (var ragType in relevantNodeTypes) {
                    if (relevantNodeTypes.hasOwnProperty(ragType)) {
                        mergedNodeTypes[ragType] = relevantNodeTypes[ragType];
                    }
                }
                
                contextNodeTypes = mergedNodeTypes;
            }
        }
