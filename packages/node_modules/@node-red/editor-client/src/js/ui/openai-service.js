...
- Always verify wire connections are correctly specified`
    ...
    function prepareMessages(userMessage, conversationHistory, availableNodeTypes, nodesInWorkspace, maxContextTokens) {
        const systemPrompt = getSystemPrompt(availableNodeTypes, nodesInWorkspace);
        ...
        // If we have a context limit, truncate history to fit
        if (availableForHistory < Infinity && conversationHistory.length > 0) {
            let historyTokens = 0;
        ...
        // No limit or limit is high enough, use recent history
        ...
        try {
            switch (provider) {
                case "openai":
                    ...
                    break;
                case "anthropic":
                    data = await callAnthropicAPI(messages, availableNodeTypes, nodesInWorkspace, model, apiKey, temperature);
                    break;
                case "gemini":
                    data = await callGeminiAPI(messages, availableNodeTypes, nodesInWorkspace, model, apiKey, temperature);
                    break;
                default:
        ...
        if (message && message.tool_calls && Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
            ...
            for (const toolCall of message.tool_calls) {
                if (toolCall.type === 'function' && toolCall.function) {
                    ...
                    if (handler) {
                        console.log('Calling handler:', name, 'with args:', functionArgs);
