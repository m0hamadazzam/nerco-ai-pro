...
- Preserve IDs for updates`
    ...
    function prepareMessages(userMessage, conversationHistory, availableNodeTypes, nodesInWorkspace, maxContextTokens) {
        ...
        if (RED.aiRAGService && RED.aiRAGService.isInitialized()) {
            ...
            if (operationType === 'question') {
                // For questions, retrieve only top 10-15 most relevant node types
                maxResults = 15;
            } else if (operationType === 'create_flow') {
                // For flow creation, retrieve top 20-30 relevant node types
                maxResults = 30;
            } else {
                // For flow updates, retrieve top 40-50 relevant node types (more context needed)
                maxResults = 50;
            }
    ...
    function init() {
        ...
        if (RED.aiRAGService) {
            ...
            // Listen for registry changes to rebuild knowledge base
            if (RED.events) {
