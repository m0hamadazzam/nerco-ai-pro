RED.chatbot = (function () {
    ...
    var tokenDisplay = null;
    var missingTypesListener = null;
    var pendingFlowInsertion = false;
    ...
    function checkApiKey() {
        ...
        for (var providerId in aiProviders) {
            if (aiProviders.hasOwnProperty(providerId)) {
    ...
    function createModelSelector() {
        ...
        modelSelector.on('change', function () {
            ...
            if (selectedValue) {
                ...
                var newProvider = parts[0];
                var newModel = parts[1];
                var newApiKeyId = parts[2];

                // Switch history when model changes
                if (RED.aiHistoryManager) {
                    RED.aiHistoryManager.switchHistory(newProvider, newModel, newApiKeyId);
                }

                selectedProvider = newProvider;
                selectedModel = newModel;
                selectedApiKeyId = newApiKeyId;
                saveSelectedModel();
    ...
    function createTokenDisplay() {
        var tokenContainer = $('<div class="red-ui-chatbot-token-display"></div>');

        var tokenWrapper = $('<div class="red-ui-chatbot-token-wrapper"></div>').appendTo(tokenContainer);

        var tokenIcon = $('<i class="fa fa-chart-line"></i>').appendTo(tokenWrapper);
        tokenIcon.css({
            'margin-right': '5px',
            'opacity': '0.7'
        });

        var tokenText = $('<span class="red-ui-chatbot-token-text">0 tokens</span>').appendTo(tokenWrapper);

        var sessionText = $('<span class="red-ui-chatbot-session-text" style="margin-left: 10px; opacity: 0.6; font-size: 11px;">Session: 0</span>').appendTo(tokenWrapper);

        var costText = $('<span class="red-ui-chatbot-cost-text" style="margin-left: 10px; opacity: 0.6; font-size: 11px;"></span>').appendTo(tokenWrapper);

        // Add reset button for token stats
        var resetButton = $('<button class="red-ui-button red-ui-button-small" style="margin-left: 10px; padding: 2px 6px; font-size: 10px;" title="Reset session stats">Reset</button>').appendTo(tokenWrapper);
        resetButton.on('click', function (e) {
            e.stopPropagation();
            if (RED.aiTokenTracker) {
                RED.aiTokenTracker.reset();
                updateTokenDisplay();
                var tokenText = tokenDisplay.find('.red-ui-chatbot-token-text');
                tokenText.text('0 tokens');
            }
        });

        // Add clear history button
        var clearHistoryButton = $('<button class="red-ui-button red-ui-button-small" style="margin-left: 5px; padding: 2px 6px; font-size: 10px;" title="Clear chat history for current model">Clear History</button>').appendTo(tokenWrapper);
        clearHistoryButton.on('click', function (e) {
            e.stopPropagation();
            if (RED.aiHistoryManager && selectedProvider && selectedModel && selectedApiKeyId) {
                if (confirm('Clear chat history for ' + selectedProvider + ' ' + selectedModel + '?')) {
                    RED.aiHistoryManager.clearHistory(selectedProvider, selectedModel, selectedApiKeyId);
                    // Clear the message container (except welcome message)
                    messageContainer.children().not(':first').remove();
                    // Reload history (will be empty now)
                    RED.aiHistoryManager.switchHistory(selectedProvider, selectedModel, selectedApiKeyId);
                }
            }
        });

        // Add tooltip with detailed stats
        tokenContainer.attr('title', 'Token usage: Current request and session totals');
    ...
    function updateTokenDisplay() {
        if (!tokenDisplay || !RED.aiTokenTracker) {
            return;
        }

        var stats = RED.aiTokenTracker.getSessionStats();
        var tokenText = tokenDisplay.find('.red-ui-chatbot-token-text');
        var sessionText = tokenDisplay.find('.red-ui-chatbot-session-text');
        var costText = tokenDisplay.find('.red-ui-chatbot-cost-text');

        // Update session total
        sessionText.text('Session: ' + formatNumber(stats.totalTokens));

        // Update cost if available
        if (stats.totalCost > 0) {
            var costStr = '$' + stats.totalCost.toFixed(4);
            if (stats.totalCost < 0.01) {
                costStr = '<$0.01';
            }
            costText.text('Cost: ' + costStr).show();
        } else {
            costText.hide();
        }

        // Token text shows last request (will be updated after each request)
        // For now, show session total
        if (stats.requestCount === 0) {
            tokenText.text('0 tokens');
        }
    }

    function updateTokenDisplayAfterRequest(tokenUsage, tokenCost) {
        if (!tokenDisplay) {
            return;
        }
        ...
        // Update current request tokens
        if (tokenUsage && tokenUsage.totalTokens) {
            tokenText.text(formatNumber(tokenUsage.totalTokens) + ' tokens');

            // Add breakdown in tooltip
            var tooltip = formatNumber(tokenUsage.promptTokens) + ' prompt + ' +
                formatNumber(tokenUsage.completionTokens) + ' completion = ' +
                formatNumber(tokenUsage.totalTokens) + ' total';
            tokenText.attr('title', tooltip);
        }

        // Update session stats
        updateTokenDisplay();

        // Briefly highlight the update
        tokenText.css('color', '#2ecc71');
        setTimeout(function () {
            tokenText.css('color', '');
        }, 1000);
    }
    ...
    function createChatbotContent() {
        ...
        // Add token display
        tokenDisplay = createTokenDisplay();
        chatContainer.append(tokenDisplay);
        ...
        if (!checkApiKey()) {
            ...
            // Initialize history for selected model
            if (RED.aiHistoryManager && selectedProvider && selectedModel && selectedApiKeyId) {
                RED.aiHistoryManager.switchHistory(selectedProvider, selectedModel, selectedApiKeyId);
            }
        }
    ...
    function addUserMessage(message) {
        ...
        if (RED.aiHistoryManager) {
            RED.aiHistoryManager.addMessage('user', message);
        }
        scrollToBottom();
    }

    function addSystemMessage(message, isWarning) {
        var systemMessage = $('<div class="red-ui-chatbot-message red-ui-chatbot-message-system"></div>')
            .appendTo(messageContainer);

        if (isWarning) {
            systemMessage.addClass('red-ui-chatbot-message-warning');
        }
        ...
        // Don't add welcome message to chat history
        if (!isWarning && RED.aiHistoryManager) {
            RED.aiHistoryManager.addMessage('assistant', message);
        }
        scrollToBottom();
    }
    ...
    function processUserMessage(message) {
        ...
        // Disable send button immediately to prevent race conditions
        var sendButton = inputContainer.find('button');
        var textarea = inputContainer.find('textarea');
        ...
        // Convert array to object keyed by node type for RAG and context
        var nodeTypes = {};
        ...
        // Ensure RAG service is initialized with current node types
        if (RED.aiRAGService && (!RED.aiRAGService.isInitialized() || Object.keys(nodeTypes).length > 0)) {
        ...
        var exportable_json = RED.nodes.createCompleteNodeSet({});

        var filtered_exportable_json = exportable_json.filter(function (node) {
            return nodesInWorkspace.some(function (n) {
                return n.id === node.id;
            });
        });
        ...
        // Ensure history is initialized for current model
        if (RED.aiHistoryManager && selectedProvider && selectedModel && selectedApiKeyId) {
            RED.aiHistoryManager.switchHistory(selectedProvider, selectedModel, selectedApiKeyId);
        }

        // Get current history
        var chatHistory = RED.aiHistoryManager ? RED.aiHistoryManager.getCurrentHistory() : [];

        RED.aiService.processMessage(message, chatHistory, nodeTypes, filtered_exportable_json, selectedProvider, selectedModel, selectedApiKeyId)
            .then(function (result) {
                processingMessage.remove();

                // Update token display
                if (result.tokenUsage) {
                    updateTokenDisplayAfterRequest(result.tokenUsage, result.tokenCost);
                }

                if (result.success) {
                    // Show truncation warning if history was truncated
                    if (result.truncationInfo) {
                        const truncInfo = result.truncationInfo;
                        addSystemMessage(`⚠️ Note: Chat history was truncated to fit the model's context window. Removed ${truncInfo.removedCount} older message(s), kept ${truncInfo.keptCount} most recent.`, true);
                    }
                    ...
                    if (result.isFlow) {
                        ...
                        // Add to chat history
                        if (RED.aiHistoryManager) {
                            RED.aiHistoryManager.addMessage('assistant', result.description, { isFlow: true });
                        }
                    }
            ...
            .catch(function (error) {
                processingMessage.remove();
                // Use RED.log if available, otherwise console.error
                if (RED.log && RED.log.error) {
                    RED.log.error("Error processing message:", error);
                } else {
    ...
    function insertFlow(flowJson) {
        ...
        if (Array.isArray(flowJson)) {
            ...
            try {
                ...
                if (RED.log && RED.log.error) {
                    RED.log.error('Failed to parse flow JSON string:', e);
                } else {
        ...
        var validFlowJson = nodesArray.filter(function (node) {
            ...
            if (!node || typeof node !== 'object' || !node.type || typeof node.type !== 'string') {
                if (RED.log && RED.log.warn) {
                    RED.log.warn('Skipping invalid node:', node);
                } else {
            ...
            if (node.type !== 'tab' && node.type !== 'subflow' && node.type !== 'group') {
                if (typeof node.x !== 'number' || typeof node.y !== 'number') {
                    if (RED.log && RED.log.warn) {
                        RED.log.warn('Skipping node without valid x/y coordinates:', node);
                    } else {
        ...
        // Set flag to track that we're inserting a flow
        pendingFlowInsertion = true;
        
        // Set up listener for missing types (will be cleaned up after handling)
        setupMissingTypesListener();
    ...
    /**
     * Set up listener for runtime-state events to detect missing node types
     */
    function setupMissingTypesListener() {
        // Remove existing listener if any
        if (missingTypesListener) {
            RED.events.off('runtime-state', missingTypesListener);
        }

        // Create new listener
        missingTypesListener = function(event) {
            // Only process missing-types events
            if (!event || !event.payload || event.payload.error !== 'missing-types') {
                return;
            }

            // Only process if we just inserted a flow
            if (!pendingFlowInsertion) {
                return;
            }

            // Reset flag
            pendingFlowInsertion = false;

            // Extract missing types
            var missingTypes = event.payload.types || [];
            if (missingTypes.length === 0) {
                return;
            }

            // Handle missing types
            handleMissingNodeTypes(missingTypes);
        };

        // Register listener
        RED.events.on('runtime-state', missingTypesListener);
    }

    /**
     * Handle missing node types by detecting modules and prompting for installation
     */
    function handleMissingNodeTypes(missingTypes) {
        if (!RED.aiModuleInstaller || !missingTypes || missingTypes.length === 0) {
            return;
        }

        // Map node types to module names
        var moduleMappings = RED.aiModuleInstaller.mapNodeTypesToModules(missingTypes);
        
        if (moduleMappings.length === 0) {
            // Could not determine module names for any missing types
            var typeList = missingTypes.join(', ');
            addSystemMessage('⚠️ The flow uses node types that are not installed: ' + typeList + 
                           '. Please install the required modules manually via the Palette Manager.', true);
            return;
        }

        // Extract module names
        var moduleNames = moduleMappings.map(function(m) {
            return m.moduleName;
        });

        // Check which modules are already installed
        var installedStatus = RED.aiModuleInstaller.checkModulesInstalled(moduleNames);
        
        // Filter out already installed modules
        var modulesToInstall = moduleMappings.filter(function(m) {
            return !installedStatus[m.moduleName];
        });

        if (modulesToInstall.length === 0) {
            // All modules are already installed, just need to refresh
            addSystemMessage('All required modules are installed. Refreshing registry...');
            // Trigger registry refresh by emitting event or calling refresh method
            if (RED.events) {
                RED.events.emit('registry:node-set-added');
            }
            return;
        }

        // Show prompt to user
        var moduleList = modulesToInstall.map(function(m) {
            return m.moduleName;
        });

        RED.aiModuleInstaller.showInstallPrompt(moduleList, function(userConfirmed) {
            if (userConfirmed) {
                installMissingModules(modulesToInstall);
            } else {
                addSystemMessage('Module installation cancelled. Some nodes may appear as "unknown" until modules are installed.', true);
            }
        });
    }

    /**
     * Install missing modules sequentially
     */
    function installMissingModules(modulesToInstall) {
        if (!modulesToInstall || modulesToInstall.length === 0) {
            return;
        }
        ...
        addSystemMessage('Installing required modules...', false);

        // Install modules sequentially
        function installNext(index) {
            if (index >= modulesToInstall.length) {
                // All installations complete
                finishModuleInstallation(installedModules, failedModules);
                return;
            }

            var moduleInfo = modulesToInstall[index];
            var moduleName = moduleInfo.moduleName;

            // Show progress
            var progressMsg = RED.aiModuleInstaller.showInstallProgress(moduleName);
            addSystemMessage(progressMsg, false);

            // Install module
            RED.aiModuleInstaller.installModule(moduleName, 'latest')
                .then(function() {
                    installedModules.push(moduleName);
                    installNext(index + 1);
                })
                .catch(function(error) {
                    var errorMsg = RED.aiModuleInstaller.handleInstallError(error, moduleName);
    ...
    /**
     * Finish module installation process
     */
    function finishModuleInstallation(installedModules, failedModules) {
        // Refresh registry to load newly installed modules
        if (installedModules.length > 0) {
            // Trigger registry refresh by emitting event
            if (RED.events) {
                RED.events.emit('registry:node-set-added');
            }

            // Show success message
            var successMsg = 'Successfully installed ' + installedModules.length + ' module(s): ' + 
                           installedModules.join(', ') + '. Nodes should now be available.';
            addSystemMessage(successMsg);
        }

        // Show error messages for failed installations
        if (failedModules.length > 0) {
            failedModules.forEach(function(failed) {
        ...
        if (missingTypesListener) {
            RED.events.off('runtime-state', missingTypesListener);
            missingTypesListener = null;
        }
    }

    function change_ids(flowJson) {
        var current_ids_to_new_ids = {};
    ...
    function init() {
        ...
        if (RED.aiHistoryManager && RED.aiHistoryManager.init) {
            RED.aiHistoryManager.init();
        }
