...
module.exports = {
    ...
    app: function() {
        ...
        // API endpoint to switch active theme dynamically
        themeApp.post("/switch", async function(req, res) {
            const newThemeId = req.body.theme || null;
            
            // Update active theme
            activeTheme = newThemeId;
            activeThemeInitialised = false;
            
            // Reset theme context for CSS
            if (theme.page && theme.page._ && theme.page._.css) {
                // Remove old theme CSS files from context
                const baseCss = themeContext.page.css || [];
                const oldThemeCss = theme.page._.css || [];
                themeContext.page.css = baseCss.filter(css => !oldThemeCss.includes(css));
                theme.page._.css = [];
            }
            
            // Get new theme CSS files
            let newCssFiles = [];
            if (newThemeId) {
                try {
                    ...
                    if (themePlugin && themePlugin.css) {
                        newCssFiles = serveFilesFromTheme(
                            themePlugin.css,
                            themeApp,
                            "/css/",
                            themePlugin.path
                        );
                        themeContext.page.css = newCssFiles.concat(themeContext.page.css || []);
                        theme.page = theme.page || {_:{}}
                        theme.page._.css = newCssFiles.concat(theme.page._.css || []);
                    }
                } catch(err) {
                    // Theme not found, continue without it
                }
            }
